...
static ConVar sk_bee_damage("sk_bee_damage", "3");
...
void CBee::Explode( void )
{
	...
	if( tr.fraction == 1.0 || !(tr.surface.flags & SURF_SKY) )
	{
		...
		CBaseEntity *pHurt = CheckTraceHullAttack(4, Vector(-2,-2,-2), Vector(2,2,2), sk_bee_damage.GetFloat(), DMG_POISON );
		if ( pHurt )
		{
...
void CBee::SeekThink( void )
{
	...
	SetNextThink( gpGlobals->curtime); //T�H�N JOKU PITEMPI AIKA
}
