...
//
// Purpose: Combine Zombie... Zombie Combine... its like a... ZOMBINEBASHER... get it?
//
// $NoKeywords: $
//=============================================================================//
...
enum
{
	SQUAD_SLOT_ZOMBINEBASHER_SPRINT1 = LAST_SHARED_SQUADSLOT,
	SQUAD_SLOT_ZOMBINEBASHER_SPRINT2,
};
...
#define SPRINT_CHANCE_VALUE 25
#define SPRINT_CHANCE_VALUE_DARKNESS 60

#define GRENADE_PULL_MAX_DISTANCE 250.0f

#define ZOMBINEBASHER_MAX_GRENADES 5

int ACT_ZOMBINEBASHER_GRENADE_PULL;
int ACT_ZOMBINEBASHER_GRENADE_WALK;
int ACT_ZOMBINEBASHER_GRENADE_RUN;
int ACT_ZOMBINEBASHER_GRENADE_IDLE;
int ACT_ZOMBINEBASHER_ATTACK_FAST;
int ACT_ZOMBINEBASHER_GRENADE_FLINCH_BACK;
int ACT_ZOMBINEBASHER_GRENADE_FLINCH_FRONT;
int ACT_ZOMBINEBASHER_GRENADE_FLINCH_WEST;
int ACT_ZOMBINEBASHER_GRENADE_FLINCH_EAST;

int AE_ZOMBINEBASHER_PULLPIN;

//extern bool IsAlyxInDarknessMode();

//ConVar	sk_zombie_soldier_health("sk_zombie_soldier_health", "0");

float g_flZOMBINEBASHERGrenadeTimes = 0;

class CNPC_ZOMBINEBASHERBasher : public CAI_BlendingHost<CNPC_BaseZombie>, public CDefaultPlayerPickupVPhysics
{
	DECLARE_DATADESC();
	DECLARE_CLASS(CNPC_ZOMBINEBASHERBasher, CAI_BlendingHost<CNPC_BaseZombie>);
...
public:
	...
	enum
	{
		COND_ZOMBINEBASHER_GRENADE = LAST_BASE_ZOMBIE_CONDITION,
		COND_ZOMBINEBASHER_SHOULD_CHARGE,
	};

	enum
	{
		SCHED_ZOMBINEBASHER_PULL_GRENADE = LAST_BASE_ZOMBIE_SCHEDULE,
	};
...
LINK_ENTITY_TO_CLASS(npc_zombinebasher, CNPC_ZOMBINEBASHERBasher);

BEGIN_DATADESC(CNPC_ZOMBINEBASHERBasher)
...
//---------------------------------------------------------
//---------------------------------------------------------
const char* CNPC_ZOMBINEBASHERBasher::pMoanSounds[] =
{
	"ATV_engine_null",
};

void CNPC_ZOMBINEBASHERBasher::Spawn(void)
{
	Precache();
	...
	CapabilitiesClear();
	CapabilitiesAdd(bits_CAP_AUTO_DOORS | bits_CAP_OPEN_DOORS | bits_CAP_MOVE_GROUND | bits_CAP_MOVE_CLIMB | bits_CAP_MOVE_JUMP);
	BaseClass::Spawn();
	...
	g_flZOMBINEBASHERGrenadeTimes = gpGlobals->curtime;
	m_flGrenadePullTime = gpGlobals->curtime;

	m_iGrenadeCount = ZOMBINEBASHER_MAX_GRENADES;
}

void CNPC_ZOMBINEBASHERBasher::Precache(void)
{
	BaseClass::Precache();

	PrecacheModel("models/zombie/zombie_soldier_basher.mdl");
	PrecacheModel("models/headcrab.mdl");
	PrecacheModel("models/headcrabclassic.mdl");

	PrecacheScriptSound("Zombie.FootstepRight");
	PrecacheScriptSound("Zombie.FootstepLeft");
	PrecacheScriptSound("ZOMBINEBASHER.ScuffRight");
	PrecacheScriptSound("ZOMBINEBASHER.ScuffLeft");
	PrecacheScriptSound("Zombie.AttackHit");
	PrecacheScriptSound("Zombie.AttackMiss");
	PrecacheScriptSound("ZOMBINEBASHER.Pain");
	PrecacheScriptSound("ZOMBINEBASHER.Die");
	PrecacheScriptSound("ZOMBINEBASHER.Alert");
	PrecacheScriptSound("ZOMBINEBASHER.Idle");
	PrecacheScriptSound("ZOMBINEBASHER.ReadyGrenade");

	PrecacheScriptSound("ATV_engine_null");
	PrecacheScriptSound("ZOMBINEBASHER.Charge");
	PrecacheScriptSound("Zombie.Attack");
}

void CNPC_ZOMBINEBASHERBasher::SetZombieModel(void)
{
	SetModel("models/zombie/zombie_soldier_basher.mdl");
...
void CNPC_ZOMBINEBASHERBasher::PrescheduleThink(void)
{
...
void CNPC_ZOMBINEBASHERBasher::OnScheduleChange(void)
{
...
bool CNPC_ZOMBINEBASHERBasher::CanRunAScriptedNPCInteraction(bool bForced)
{
	if (HasGrenade() == true)
		return false;
...
int CNPC_ZOMBINEBASHERBasher::SelectSchedule(void)
{
	if (GetHealth() <= 0)
		return BaseClass::SelectSchedule();

	if (HasCondition(COND_ZOMBINEBASHER_GRENADE))
	{
		ClearCondition(COND_ZOMBINEBASHER_GRENADE);

		return SCHED_ZOMBINEBASHER_PULL_GRENADE;
	}

	return BaseClass::SelectSchedule();
}

void CNPC_ZOMBINEBASHERBasher::BuildScheduleTestBits(void)
{
	BaseClass::BuildScheduleTestBits();

	SetCustomInterruptCondition(COND_ZOMBINEBASHER_GRENADE);
}

Activity CNPC_ZOMBINEBASHERBasher::NPC_TranslateActivity(Activity baseAct)
{
	if (baseAct == ACT_MELEE_ATTACK1)
	{
		if (m_flSuperFastAttackTime > gpGlobals->curtime || HasGrenade())
		{
			return (Activity)ACT_ZOMBINEBASHER_ATTACK_FAST;
		}
	}
	...
	if (baseAct == ACT_IDLE)
	{
		if (HasGrenade())
		{
			return (Activity)ACT_ZOMBINEBASHER_GRENADE_IDLE;
		}
	}
...
int CNPC_ZOMBINEBASHERBasher::MeleeAttack1Conditions(float flDot, float flDist)
{
...
void CNPC_ZOMBINEBASHERBasher::GatherGrenadeConditions(void)
{
	if (m_iGrenadeCount <= 0)
		return;

	if (g_flZOMBINEBASHERGrenadeTimes > gpGlobals->curtime)
		return;
	...
	if (pPlayer && pPlayer->FVisible(this))
	{
		...
		if (flLengthToPlayer <= GRENADE_PULL_MAX_DISTANCE || flLengthToEnemy <= GRENADE_PULL_MAX_DISTANCE)
		{
			...
			{
				g_flZOMBINEBASHERGrenadeTimes = gpGlobals->curtime + 10.0f;
				SetCondition(COND_ZOMBINEBASHER_GRENADE);
			}
		}
	}
}

int CNPC_ZOMBINEBASHERBasher::TranslateSchedule(int scheduleType)
{
	return BaseClass::TranslateSchedule(scheduleType);
}

void CNPC_ZOMBINEBASHERBasher::DropGrenade(Vector vDir)
{
	if (m_hGrenade == NULL)
		return;
...
void CNPC_ZOMBINEBASHERBasher::Event_Killed(const CTakeDamageInfo& info)
{
...
//-----------------------------------------------------------------------------
bool CNPC_ZOMBINEBASHERBasher::HandleInteraction(int interactionType, void* data, CBaseCombatCharacter* sourceEnt)
{
...
void CNPC_ZOMBINEBASHERBasher::TraceAttack(const CTakeDamageInfo& info, const Vector& vecDir, trace_t* ptr, CDmgAccumulator* pAccumulator)
{
...
void CNPC_ZOMBINEBASHERBasher::HandleAnimEvent(animevent_t* pEvent)
{
	if (pEvent->event == AE_ZOMBINEBASHER_PULLPIN)
	{
		Vector vecStart;
		QAngle angles;
		...
		if (pGrenade)
		{
			...
			if (pPhysicsObject)
			{
				...
				EmitSound("ZOMBINEBASHER.ReadyGrenade");
...
bool CNPC_ZOMBINEBASHERBasher::AllowedToSprint(void)
{
	if (IsOnFire())
		return false;
	...
	{
		if (IsStrategySlotRangeOccupied(SQUAD_SLOT_ZOMBINEBASHER_SPRINT1, SQUAD_SLOT_ZOMBINEBASHER_SPRINT2) == true)
			return false;
...
void CNPC_ZOMBINEBASHERBasher::StopSprint(void)
{
...
//=========================================================
int CNPC_ZOMBINEBASHERBasher::OnTakeDamage_Alive(const CTakeDamageInfo& inputInfo)
{
	...
	// if the ZOMBINEBASHER is running, has an enemy, was hurt by the enemy, hasn't been hurt in the last 3 seconds, and isn't too close to the enemy,
	// it will swerve. (whew).
	...
	if (GetEnemy() != NULL && IsMoving() && pevAttacker == GetEnemy() && gpGlobals->curtime - m_flLastHurtTime > 3)
	{
		...
		if (flDist > MIN_SPRINT_DISTANCE)
		{
			...
			if (pRoute)
			{
				...
				vecApex = Vector(random->RandomInt(0, 2), 0, random->RandomInt(0, 2));//GetPathfinder()->BuildTriangulationRoute(GetAbsOrigin(), pRoute[pRoute->iNodeID].vecLocation, GetEnemy(), NULL,NULL, 0.0, flDist * 0.5, NAV_GROUND)->GetPos();
		...
		if (FClassnameIs(inputInfo.GetAttacker(), "monster_assassin") || FClassnameIs(inputInfo.GetAttacker(), "monster_bullsquid"))
		{
			SetEnemy(inputInfo.GetAttacker());
...
void CNPC_ZOMBINEBASHERBasher::Sprint(bool bMadSprint)
{
	if (IsSprinting())
		return;

	OccupyStrategySlotRange(SQUAD_SLOT_ZOMBINEBASHER_SPRINT1, SQUAD_SLOT_ZOMBINEBASHER_SPRINT2);
	...
	EmitSound("ZOMBINEBASHER.Charge");
}

void CNPC_ZOMBINEBASHERBasher::RunTask(const Task_t* pTask)
{
	switch (pTask->iTask)
	{
	...
	case TASK_WAIT_FOR_MOVEMENT:
	{
		...
		if (GetEnemy())
		{
			...
			if (HasGrenade())
			{
				if (IsSprinting())
				{
					GetNavigator()->SetMovementActivity((Activity)ACT_ZOMBINEBASHER_GRENADE_RUN);
				}
				else
				{
					GetNavigator()->SetMovementActivity((Activity)ACT_ZOMBINEBASHER_GRENADE_WALK);
				}
...
void CNPC_ZOMBINEBASHERBasher::InputStartSprint(inputdata_t& inputdata)
{
	Sprint();
}

void CNPC_ZOMBINEBASHERBasher::InputPullGrenade(inputdata_t& inputdata)
{
	g_flZOMBINEBASHERGrenadeTimes = gpGlobals->curtime + 5.0f;
	SetCondition(COND_ZOMBINEBASHER_GRENADE);
}
...
//-----------------------------------------------------------------------------
const char* CNPC_ZOMBINEBASHERBasher::GetMoanSound(int nSound)
{
...
//-----------------------------------------------------------------------------
void CNPC_ZOMBINEBASHERBasher::FootstepSound(bool fRightFoot)
{
	if (fRightFoot)
	{
...
//-----------------------------------------------------------------------------
// Purpose: Overloaded so that explosions don't split the ZOMBINEBASHER in twain.
//-----------------------------------------------------------------------------
bool CNPC_ZOMBINEBASHERBasher::ShouldBecomeTorso(const CTakeDamageInfo& info, float flDamageThreshold)
{
	return false;
}
...
//-----------------------------------------------------------------------------
void CNPC_ZOMBINEBASHERBasher::FootscuffSound(bool fRightFoot)
{
	if (fRightFoot)
	{
		EmitSound("ZOMBINEBASHER.ScuffRight");
	}
	else
	{
		EmitSound("ZOMBINEBASHER.ScuffLeft");
	}
}
...
//-----------------------------------------------------------------------------
void CNPC_ZOMBINEBASHERBasher::AttackHitSound(void)
{
...
//-----------------------------------------------------------------------------
void CNPC_ZOMBINEBASHERBasher::AttackMissSound(void)
{
...
// Purpose: 
//-----------------------------------------------------------------------------
void CNPC_ZOMBINEBASHERBasher::PainSound(const CTakeDamageInfo& info)
{
	...
	EmitSound("ZOMBINEBASHER.Pain");
}

//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CNPC_ZOMBINEBASHERBasher::DeathSound(const CTakeDamageInfo& info)
{
	EmitSound("ZOMBINEBASHER.Die");
}

//-----------------------------------------------------------------------------
// Purpose: 
//-----------------------------------------------------------------------------
void CNPC_ZOMBINEBASHERBasher::AlertSound(void)
{
	EmitSound("ZOMBINEBASHER.Alert");
...
//-----------------------------------------------------------------------------
void CNPC_ZOMBINEBASHERBasher::IdleSound(void)
{
...
//-----------------------------------------------------------------------------
void CNPC_ZOMBINEBASHERBasher::AttackSound(void)
{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
const char* CNPC_ZOMBINEBASHERBasher::GetHeadcrabModel(void)
{
...
// Purpose: 
//-----------------------------------------------------------------------------
const char* CNPC_ZOMBINEBASHERBasher::GetLegsModel(void)
{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
const char* CNPC_ZOMBINEBASHERBasher::GetTorsoModel(void)
{
...
//---------------------------------------------------------
void CNPC_ZOMBINEBASHERBasher::MoanSound(envelopePoint_t* pEnvelope, int iEnvelopeSize)
{
	if (IsOnFire())
	{
...
//-----------------------------------------------------------------------------
const char* CNPC_ZOMBINEBASHERBasher::GetHeadcrabClassname(void)
{
	return "npc_headcrab_fast";
}

void CNPC_ZOMBINEBASHERBasher::ReleaseGrenade(Vector vPhysgunPos)
{
	if (HasGrenade() == false)
		return;
	...
	if (flDotRight > flDotForward)
	{
		if (bNegativeRight == true)
			aActivity = (Activity)ACT_ZOMBINEBASHER_GRENADE_FLINCH_WEST;
		else
			aActivity = (Activity)ACT_ZOMBINEBASHER_GRENADE_FLINCH_EAST;
	}
	else
	{
		if (bNegativeForward == true)
			aActivity = (Activity)ACT_ZOMBINEBASHER_GRENADE_FLINCH_BACK;
		else
			aActivity = (Activity)ACT_ZOMBINEBASHER_GRENADE_FLINCH_FRONT;
	}
...
CBaseEntity* CNPC_ZOMBINEBASHERBasher::OnFailedPhysGunPickup(Vector vPhysgunPos)
{
...
AI_BEGIN_CUSTOM_NPC(npc_zombinebasher, CNPC_ZOMBINEBASHERBasher)

//Squad slots
DECLARE_SQUADSLOT(SQUAD_SLOT_ZOMBINEBASHER_SPRINT1)
DECLARE_SQUADSLOT(SQUAD_SLOT_ZOMBINEBASHER_SPRINT2)

DECLARE_CONDITION(COND_ZOMBINEBASHER_GRENADE)

DECLARE_ACTIVITY(ACT_ZOMBINEBASHER_GRENADE_PULL)
DECLARE_ACTIVITY(ACT_ZOMBINEBASHER_GRENADE_WALK)
DECLARE_ACTIVITY(ACT_ZOMBINEBASHER_GRENADE_RUN)
DECLARE_ACTIVITY(ACT_ZOMBINEBASHER_GRENADE_IDLE)
DECLARE_ACTIVITY(ACT_ZOMBINEBASHER_ATTACK_FAST)
DECLARE_ACTIVITY(ACT_ZOMBINEBASHER_GRENADE_FLINCH_BACK)
DECLARE_ACTIVITY(ACT_ZOMBINEBASHER_GRENADE_FLINCH_FRONT)
DECLARE_ACTIVITY(ACT_ZOMBINEBASHER_GRENADE_FLINCH_WEST)
DECLARE_ACTIVITY(ACT_ZOMBINEBASHER_GRENADE_FLINCH_EAST)

DECLARE_ANIMEVENT(AE_ZOMBINEBASHER_PULLPIN)


DEFINE_SCHEDULE
(
	SCHED_ZOMBINEBASHER_PULL_GRENADE,

	"	Tasks"
	"		TASK_PLAY_SEQUENCE					ACTIVITY:ACT_ZOMBINEBASHER_GRENADE_PULL"
